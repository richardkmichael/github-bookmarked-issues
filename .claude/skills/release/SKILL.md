...
- RC tags: `vX.Y.Z-rcN` — no package.json bump
- Final tags: `vX.Y.Z` — bump package.json + package-lock.json, commit first
- Conventional commits inform semver assessment: `feat:` → minor, `BREAKING CHANGE:` → major, else → patch

## Release Workflow Integration

Pushing a tag triggers `.github/workflows/release.yml`, which runs tests, builds
both extensions (Chrome .zip + Firefox .xpi), and uploads the artifacts to the
GitHub release that this skill already created.

The skill creates the release (with custom notes); the workflow only uploads artifacts.
...
4. Present proposed next tag and confirm with user:
   - If RC series already in progress (e.g. `v1.0.0-rc3`): default is `v1.0.0-rc4`; also show semver suggestion in case user wants to revise the version base
   - If starting fresh from a final tag: show all three options (`vX.Y.Z-rc1` for patch/minor/major)
5. Get commits for changelog: `git log <last-rc-or-final>..HEAD --oneline` (incremental — since last RC, or since last final if this is rc1)
...
`**Full Changelog**: https://github.com/richardkmichael/github-bookmarked-issues/compare/<from>...<to>`
...
**Full Changelog**: https://github.com/richardkmichael/github-bookmarked-issues/compare/v1.0.0-rc3...v1.0.0-rc4
```
...
## Breaking Changes
- Minimum Firefox version now 109 (Manifest V3 support)

**Full Changelog**: https://github.com/richardkmichael/github-bookmarked-issues/compare/v1.0.0...v1.1.0-rc1
```
